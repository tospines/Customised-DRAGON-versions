...
//const double TSpallationNetwork::ethr = 5.e1;
  ...
  TSpallationNetwork::TSpallationNetwork(TGrid* coord, Input* in, std::vector<TXSecBase*> xsecmodel, std::vector<int>& nuclei) {
	...
	Eap[i] = Elept[i];
      ...
      //for (int i = 0; i <= dimEn-1; i++) Eap[i] = pow(10., log10(1e-2)+double(i)*DBlog+0.5*DBlog);  
	...
	for (unsigned int i = 0; i < dimEn; i++){	  
	  ...
	  //std::cout << ET[0] << " " << Epr << " " << ET[dimprot-1] << " " << Ep_i << " " << Ep[0] << std::endl; 
	  ...
	  //std::cout << "First: " << cs_pHe << "with vec: " << cs_pp <<std::endl;;
	  if ((cs_pHe < 0) || (cs_pp < 0)){std::cout << "cs_pHe: " << cs_pHe << ", cs_pp: " << cs_pp <<std::endl;}
	...
	if (in->apy == Winkler){
	  ...
	  for (unsigned int j = 0; j < dimEn; j++){
	    ...
	    for (unsigned int i = 0; i < dimEn; i++){
	      ...
	      //std::cout << ET[0] << " " << Epr << " " << ET[dimprot-1] << " " << Eap_i << " " << Eap[0] << " " << Eap[dimprot-1] << std::endl; 
	...
	if(in->ly == FlukaLep || in->ly == GalpropTable){
	  ...
	  for (unsigned int j=0; j<dimEn; j++){
	    ...
	    for (unsigned int i = 0; i < dimEn; i++) {
	      ...
	      //std::cout << Eel << " " << Elept[dimlept-1] << " " << Elept[0] << " " << ET[0] << " " << ET[dimpr-1] << std::endl;
  ...
  double Eel;
   ...
   for (unsigned int j=0; j< dimEn; j++){
    ...
    for (unsigned int i = 0; i < dimEn; i++) {
      ...
      cs_pHe = pow(4.0,2.0/3.0)*KamaeYields::GetSigma(Eel, Epr, ID_ELECTRON);
      
      cs_Hep = pow(4.0,2.0/3.0)*KamaeYields::GetSigma(Eel, Epr, ID_ELECTRON);
      
      cs_HeHe = pow(4.0*4.0,2.0/3.0)*KamaeYields::GetSigma(Eel, Epr, ID_ELECTRON);  
...
// Inelastic cross sections
  TInelasticCrossSection::TInelasticCrossSection(TGrid* Coord, Input* in, int uid, int K_electron, std::vector<TXSecBase*> xsecmodel) { //double* coord, std::string* file, int uid, int K_electron){
	...
	std::cout << "\nFluka inelastic XSec OK!!\n" << std::endl;
      ...
      /*
      //if (outfileX.is_open() == false) {outfileX.open("IneHXS.dat");}      
      for(int ip = 0; ip < e_vec.size(); ip++){      
      //outfileX << "Zi.Ai\t" << "Zo.Ao\t\t" <<"Energy(GeV)\t\t" << "XSec(mb)\n";
      ...
      std::cout << jz << "." << ja << "\t\t" << e_vec[ip] << "\t\t" <<  totalxsec[ip]/beta[ip]/factor << "\n"; 
      } 
      */
