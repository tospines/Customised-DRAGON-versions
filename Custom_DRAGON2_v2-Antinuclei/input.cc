...
int Input::QueryIntAttributeWithDefault(string obj, TiXmlElement* el, int def) {
	TiXmlElement* el1 = el->FirstChildElement(obj);
	int result = def;
	if (el1) el1->QueryIntAttribute("value", &result);
	return result;
...
string Input::QueryStringAttributeWithDefault(string obj, TiXmlElement* el, string def) {
	TiXmlElement* el1 = el->FirstChildElement(obj);
	string result = def;
	if (el1) el1->QueryStringAttribute("value", &result);
	return result;
...
int Input::LoadFile(const string inputfilename) {
	...
	if (doc->LoadFile(inputfilename.c_str(), TIXML_DEFAULT_ENCODING)) {
		...
		if (el1){
			...
			if (feedback >0) cout<<"You can specify a <Observer> block with x y and z coordinates if you want to change these values" << endl;
		...
		if (el1) {
			...
			prop_secAdeuteron = ( el1->FirstChildElement("PropSecAntiDeuteron") );
			prop_secAHe3 = ( el1->FirstChildElement("PropSecAntiHe3") );
			prop_secAHe4 = ( el1->FirstChildElement("PropSecAntiHe4") );
		...
		if  (el1) {
			DoubleRun = true;
			if (feedback >0) cout << "A second iteration of the nuclear chain will be performed in order to take the beta- decays into account properly. " << endl;
		}
		...
		if (!el) {
			...
			if (OpSplit) {
				...
				MaxIter = QueryIntAttributeWithDefault("MaxIter", el1, 1e10);
			}
		...
		if      (gmod == "Galprop") ISRF_model = GalpropISRF;
			...
			cout << "implementing the standard Galprop-like ISRF " << endl;
		...
		if (el1)
		{
			...
			delta_L = QueryDoubleAttributeWithDefault("delta_L", el1, delta);
			delta_H = QueryDoubleAttributeWithDefault("delta_H", el1, delta);
			rho_H = QueryDoubleAttributeWithDefault("rho_H", el1, 5001.);
			rho_L = QueryDoubleAttributeWithDefault("rho_L", el1, 0.);
			s_H = QueryDoubleAttributeWithDefault("s_H", el1, 0.04);
			s_L = QueryDoubleAttributeWithDefault("s_L", el1, 0.04);
			
			Disk_table = QueryStringAttribute("Disk_table", el1);
			Halo_table = QueryStringAttribute("Halo_table", el1);
			normDisk = QueryDoubleAttributeWithDefault("normDisk", el1, 1.);
			normHalo = QueryDoubleAttributeWithDefault("normHalo", el1, 1.);

			BreakDiff = el1->FirstChildElement("BreakDiff");
			Double_b = el1->FirstChildElement("Double_Break");
			LowE_b = el1->FirstChildElement("LowE_Break");
			VariableD = el1->FirstChildElement("VariableD");
			TwoZoneModel = el1->FirstChildElement("TwoZone");
			Diff_table = el1->FirstChildElement("Diff_table");
			OneZone_interp = el1->FirstChildElement("OneZone_interp");
			VariableVA = el1->FirstChildElement("VariableVA");
			
			diff_threshold = QueryDoubleAttributeWithDefault("DiffusionThreshold",el1,12.);
			
			if (VariableD == true) {
				if (TwoZoneModel == true){
					Delta_delta = QueryDoubleAttributeWithDefault("Delta_delta",el1, 0.);
				}
				...
				L_connecting = QueryDoubleAttributeWithDefault("L_connecting",el1, 0.050);
				
			}

			disk_size = QueryDoubleAttributeWithDefault("disk_size",el1, 0.15*zmax);
		...
		if (!el1) REACC = false;
			...
			//            cout << QueryDoubleAttribute("vA_kms", el1) << endl;
		...
		if (feedback >0) cout << "Convection settings read " << endl;
		...
		if (!el1) cerr << "You did not specify a magnetic field model. A null magnetic field will be used." << endl;
			...
			if (feedback >0) cout << "B0 disk/halo/turb = " << B0disk << " " << B0halo << " " << B0turb << endl;	
		...
		if (el1) {
		  ...
		  el1->QueryStringAttribute("type", &gmod);
		  if (gmod == "GalpropXSec")  spallationxsec = GalpropXSec;
		  else if (gmod == "Webber03") spallationxsec = Webber03;
		  else if (gmod == "DRAGON2") spallationxsec = DRAGON2;
		  else if (gmod == "Fluka") {
		    spallationxsec = Fluka;
		    // cout << "Fluka E grid " << Ekmin << " " << Ekmax << endl;
		    // Ekfact = pow(10., 1./16.);
				// if(Ekmin <= 0.01) {
				//   Ekmin = 0.01;
				// } else {
				//   int iemin = int(log10(Ekmin/0.01)*16.);
				//   Ekmin = 0.01*pow(10., float(iemin)/16.);
				// }
				// if(Ekmax >= 0.01*pow(Ekfact, 160.)){
				//   Ekmax = 0.01*pow(Ekfact, 160.);
				// } else {
				//   int iemax = int(log10(Ekmax/0.01)*16.)+1;
				//   if(iemax>160) {
				//     iemax = 160;
				//   }
				//   cout << "iemax = " << iemax << endl;
				//   Ekmax = 0.01*pow(10., float(iemax)/16.);
				// }
				// cout << "Fluka E grid " << Ekmin << " " << Ekmax << endl;
		  }
		  else {
		    ...
		    //exit(XSECMODEL);
		    cerr << "Setting the default XSec model: DRAGON2 algorithm" << endl;
		    spallationxsec = DRAGON2;
		  }

		  BXS_Scale = QueryDoubleAttributeWithDefault("B_XS_Scal", el1, 1.);
		  BeXS_Scale = QueryDoubleAttributeWithDefault("Be_XS_Scal", el1, 1.);
		  LiXS_Scale = QueryDoubleAttributeWithDefault("Li_XS_Scal", el1, 1.);
		  FXS_Scale = QueryDoubleAttributeWithDefault("F_XS_Scal", el1, 1.);

		  el1->QueryStringAttribute("leptopt", &gmod);
		  if (gmod == "GalpropTable") ly = GalpropTable;
		  else if (gmod == "Kamae") ly = Kamae;
		  else if (gmod == "LeptUser"){ ly = LeptUser;
		    User_pp_El = QueryStringAttribute("User_El_pp", el1);
		    User_pHe_El = QueryStringAttribute("User_El_pHe", el1);
		    User_Hep_El = QueryStringAttribute("User_El_Hep", el1);
		    User_HeHe_El = QueryStringAttribute("User_El_HeHe", el1);
		    
		    User_pp_Pos = QueryStringAttribute("User_Pos_pp", el1);
		    User_pHe_Pos = QueryStringAttribute("User_Pos_pHe", el1);
		    User_Hep_Pos = QueryStringAttribute("User_Pos_Hep", el1);
		    User_HeHe_Pos = QueryStringAttribute("User_Pos_HeHe", el1);
		  }
		  else if (gmod == "Pohl") ly = Pohl;
		  else if (gmod == "Fluka") ly = FlukaLep;
		  else {
		    if (feedback >0) cout << "You did not choose any lepton yield. It will be set to Kamae." << endl;
		    ly = Kamae;
		  }
		  El_A_Scal = QueryDoubleAttributeWithDefault("El_A_Scal", el1, 0.9);
		  Pos_A_Scal = QueryDoubleAttributeWithDefault("Pos_A_Scal", el1, 0.9);
		  ...
		  else if (gmod == "Winkler") apy = Winkler;
		  else {
		    if (feedback >0) cout << "You did not choose any antiproton yield. It will be set to Winkler." << endl;
		    apy = Winkler;
		  }
		  Ap_A_Scal = QueryDoubleAttributeWithDefault("Ap_A_Scal", el1, 0.9);
		  Ap_XS_Scal = QueryDoubleAttributeWithDefault("Ap_XS_Scal", el1, 1.);
		  
		  el1->QueryStringAttribute("ANopt", &gmod);
		  if (gmod == "DRAGONAN") any = DRAGONAN;
		  else if (gmod == "UserAN"){ any = UserAN;
		    AN_DRAGON_IneXSec = el1->FirstChildElement("DRAGON_IneXSec") ; //Use DRAGON Ine XSecs
		    if (prop_secAdeuteron == true){
		      Scale_pAd = el1->FirstChildElement("Scale_pAd");
		  		      
		      Adtable_pp = QueryStringAttributeWithDefault("Adtable_pp", el1, "data/DRAGON_dbar_pp.dat");
		      Adtable_pHe = QueryStringAttributeWithDefault("Adtable_pHe", el1,"data/DRAGON_dbar_pHe.dat");
		      Adtable_app = QueryStringAttributeWithDefault("Adtable_app", el1, "null");
		      Adtable_apHe = QueryStringAttributeWithDefault("Adtable_apHe", el1,"null");
		      Adtable_Hep = QueryStringAttributeWithDefault("Adtable_Hep", el1, "data/DRAGON_dbar_Hep.dat");
		      Adtable_HeHe = QueryStringAttributeWithDefault("Adtable_HeHe", el1,"data/DRAGON_dbar_HeHe.dat");
		    }

		    if (prop_secAHe3 == true){
		      Scale_pAHe3 = el1->FirstChildElement("Scale_pAHe3");
		      		      
		      AHe3table_pp = QueryStringAttributeWithDefault("AHe3table_pp", el1, "data/DRAGON_hebar_pp.dat");
		      AHe3table_pHe = QueryStringAttributeWithDefault("AHe3table_pHe", el1,"data/DRAGON_hebar_pHe.dat");
		      AHe3table_app = QueryStringAttributeWithDefault("AHe3table_app", el1, "null");
		      AHe3table_apHe = QueryStringAttributeWithDefault("AHe3table_apHe", el1,"null");
		      AHe3table_Hep = QueryStringAttributeWithDefault("AHe3table_Hep", el1, "data/DRAGON_hebar_Hep.dat");
		      AHe3table_HeHe = QueryStringAttributeWithDefault("AHe3table_HeHe", el1,"data/DRAGON_hebar_HeHe.dat");
		    }
		    /*
		      if (prop_secAHe4 == true){
		      Scale_pAHe4 = el1->FirstChildElement("Scale_pAHe4");
		      AntiD_DRAGON_IneXSec = el1->FirstChildElement("DRAGON_IneXSec") ; //Use DRAGON Ine XSecs
		      
		      AHe4table_pp = QueryStringAttributeWithDefault("AHe4table_pp", el1, "DRAGON_AHe4table_pp.dat");
		      AHe4table_pHe = QueryStringAttributeWithDefault("AHe4table_pHe", el1,"DRAGON_AHe4table_pHe.dat");
		      AHe4table_app = QueryStringAttributeWithDefault("AHe4table_app", el1, "DRAGON_AHe4table_app.dat");
		      AHe4table_apHe = QueryStringAttributeWithDefault("AHe4table_apHe", el1,"DRAGON_AHe4table_apHe.dat");
		      AHe4table_Hep = QueryStringAttributeWithDefault("AHe4table_Hep", el1, "DRAGON_AHe4table_Hep.dat");
		      AHe4table_HeHe = QueryStringAttributeWithDefault("AHe4table_HeHe", el1,"DRAGON_AHe4table_HeHe.dat");
		      }
		    */
		  }
		  else {
		    if (feedback >0) cout << "You did not choose any antinuclei yield. It will be set to DRAGONAN." << endl;
		    any = DRAGONAN;
		  }	
		...
		if(el1)
		  {
		    ...
		    if (SA_type == "Daniele" || SA_type == "Blasi" || SA_type == "BlasiModel" || SA_type == "Standard") {
		...
		if (!el) {
		  ...
		  if (gmod == "PowerLawBreak") Spectrum = PowerLawBreak;
		    ...
		    if (feedback >0) cout << "You did not choose any spectral shape. It will be set to PowerLawBreak." << endl;
		  ...
		  if (prop_nuclei==true) {
		    ...
		    if (!el1){
		      if (feedback >0) cout << "You did not specify an InjectionIndexAllNuclei, using .source.param values." << endl;
		    }
		    else if (el1) {
		      UseInjectionIndexAllNuclei = true;
		      cout << "Using the same injection index for all species from xml file " << endl;
		  ...
		  cout << "norm energy of the extra component = " << sp_ref_rig_el_extra << endl;
		  ...
		  cout << "norm energy of the extra component = " << spect_norm_el_extra << endl;
		...
		if (el) {
		  ...
		  prop_DMHe3 = ( el->FirstChildElement("PropDMAntiHe3") );
		  prop_DMHe4 = ( el->FirstChildElement("PropDMAntiHe4") );
		  ...
		  if (gmod == "Delta") {DMs = Delta; cout << "delta injection! " <<endl;}
		    ...
		    DMs = SelfTable;
		    if (prop_DMap){cout<<"Using user defined table for DM ap"<<endl;
		    ...
		    if (prop_DMHe3)
		      MySelfTableDMHe3bar = QueryStringAttribute("AntiHe3Datafile", el);
		    if (prop_DMHe4)
		      MySelfTableDMHe4bar = QueryStringAttribute("AntiHe4Datafile", el);
		    if (prop_DMel){cout<<"Using user defined table for DM leptons"<<endl;
		...
		prop_Adeuteron = (prop_secAdeuteron || prop_DMdeuteron);
		prop_AHe3 = (prop_secAHe3 || prop_DMHe3);
		prop_AHe4 = (prop_secAHe4 || prop_DMHe4);
...
void Input::Print() {
	...
	cout << "Whether to propagate antideuterons " << prop_Adeuteron << endl;
	cout << "Whether to propagate antiHe3 " << prop_AHe3 << endl;
	cout << "Whether to propagate antiHe4 " << prop_AHe4 << endl;
