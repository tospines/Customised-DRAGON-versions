...
//2D engine -------------------------------------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------------------------------------------   
void TCREvolutor::Run(vector<double>& N, vector<double>& N_previous, TInelasticCrossSection *xsec_object, const vector<double>& dpdt, const vector<double>& SecSource_, const vector<double>& spectrum, double A, double Z, double lifetime, int daughter, std::vector<int>& nuclei, bool SecEl, int K_electron, bool isDM, bool isextra) {
//-----------------------------------------------------------------------------------------------------------------------------------   

  if (in->feedback >0) cout << "This is the 2D engine! " << endl;

  int uid;
  if ( (Z > -1) & (A>0)) uid = 1000*Z+A;
  else uid = -1000+A; 
  
  int nupart;

  for (int nuloop = 0; nuloop < nuclei.size(); ++nuloop){ if(uid == nuclei[nuloop]) nupart = nuloop; }
  ...
  double injfactor = CalcInjFactor(K_electron, SecEl, isDM, A, Z);
   ...
   if (uid == -997 || uid == -998){
     for (int i = 0; i < dimE; ++i)
       //if (SecSource_[coord->index(int(dimr/2),int(dimz/2))*dimE+i] > 1e-30)
       std::cout << "  Density!\n\n  " << SecSource_[index(int(dimr/2),int(dimz/2), i)] <<  "   , spectrum " << spectrum[i] << "  , Dperp " <<  gal->GetDiffCoeff()->GetSpectrum(i, nupart) <<  "  ,Gas " << gal->GetTotalGas()->GetGas(index(int(dimr/2),int(dimz/2),i)) << "   , Source: " << GetSourceTerm(isDM, isextra)->GetSource(index(int(dimr/2),int(dimz/2), i)) << std::endl;}
   */
   ...
   cout << "inelastic xsec loaded" << endl;
   ...
   //cout << "\n\n\n SITEEEEEEE CREVOLL.CC Dperp" << dpp->GetSpectrum(10, 2) << " " << dpp->GetSpectrum(10, 3)  << " " << dpp->GetSpectrum(10, 4) << endl;
   //exit(-1);
   //std::cout << "\n\nNUPART SITEEEEEE " << (dpp->GetReaccelerationCoefficient(0)) << std::endl;
   ...
   */   
   ...
   for (int k = 0; k < dimE; k++){  dperpfactor[k] = 1.0; dppfactor[k]=1.0;}
...
#pragma omp parallel for default(shared) private(i,j,k,indspat,ind) schedule(dynamic) num_threads(OMP_NUM_THREADS)
   ...
   for (i = 0; i < dimr; i++) {
      for (j = 0; j < dimz; j++) {
	   ...
	   if (dpp) {
	     double dppfix = dpp1*dpp->GetSpectrum(k, nupart);
	     double dppdown = dpp1*dpp->GetSpectrum(k-1, nupart);
   ...
   /*cout << "\n It was the GETREACCELERATIONCOEFFICIENT \n\n";
   
   std::cout << "\n\n\nGETSPECTRUM of REACCELERATIONCOEFFICIENT: ";
   for (k = 0; k < dimE; k++) {        
     //std::cout << "\n\n\nGETSPECTRUM of REACCELERATIONCOEFFICIENT: ";
     std::cout << dpp->GetSpectrum(k, nupart) << ", " ;
   }
   */
   ...
   /*
     std::cout << "\n\n\nGETSPECTRUM of dperp: ";
     for(int e=0;e<dimE;e++) std::cout << dperp->GetSpectrum(e, nupart) << ", "  ;
     std::cout << "\n ";
     
     
     std::cout << "\nGETCNdiff_3_r\n" << dperp->GetCNdiff_alpha3_r(0, nupart) << ", " << dperp->GetCNdiff_alpha3_r(1, nupart) << ", " << dperp->GetCNdiff_alpha3_r(2, nupart) << ", " << dperp->GetCNdiff_alpha3_r(3, nupart) << ", " << dperp->GetCNdiff_alpha3_r(10, nupart) << ", " << dperp->GetCNdiff_alpha3_r(20, nupart) << ", " << dperp->GetCNdiff_alpha3_r(30, nupart) << ", " << dperp->GetCNdiff_alpha3_r(100, nupart) << ", " << dperp->GetCNdiff_alpha3_r(200, nupart) << ", " << dperp->GetCNdiff_alpha3_r(88, nupart) << ", " << dperp->GetCNdiff_alpha3_r(89, nupart) << ", " << endl;   
     std::cout << "\n ";
     
     std::cout << "\n GetCNdiff_3_z\n" << dperp->GetCNdiff_alpha3_z(0, nupart) << ", " << dperp->GetCNdiff_alpha3_z(1, nupart) << ", " << dperp->GetCNdiff_alpha3_z(2, nupart) << ", " << dperp->GetCNdiff_alpha3_z(3, nupart) << ", " << dperp->GetCNdiff_alpha3_z(10, nupart) << ", " << dperp->GetCNdiff_alpha3_z(20, nupart) << ", " << dperp->GetCNdiff_alpha3_z(30, nupart) << ", " << dperp->GetCNdiff_alpha3_z(100, nupart) << ", " << dperp->GetCNdiff_alpha3_z(200, nupart) << ", " << dperp->GetCNdiff_alpha3_z(88, nupart) << ", " << dperp->GetCNdiff_alpha3_z(89, nupart) << ", " << endl;   
     std::cout << "\n ";
     //exit(54);
     */
   
   while (dt > in->dtmin  && (count_temp*in->Nrept < in->MaxIter)) {
     dtbar = dt/p;
...
#pragma omp for schedule(dynamic)
	 ...
	 for (ip = 0; ip < dimE; ip++) {
	   ...
	   double sp = dperp->GetSpectrum(ip, nupart);
	   ...
	   for (j = 0; j < dimr-1; j++) {
	     for (i = 0; i < dimz; i++) {
	       ...
	       double CNalphaz1 = dperp->GetCNdiff_alpha1_z(ind, nupart);
	       double CNalphaz2 = dperp->GetCNdiff_alpha2_z(ind, nupart);
	       double CNalphaz3 = dperp->GetCNdiff_alpha3_z(ind, nupart);
	 ...
	 for (ip = 0; ip < dimE; ip++) {
	   ...
	   for (j = 1; j < dimz-1; j++) {
	     for (i = 0; i < dimr; i++) {
	       ...
	       double CNalphar1 = dperp->GetCNdiff_alpha1_r(ind, nupart);
	       double CNalphar2 = dperp->GetCNdiff_alpha2_r(ind, nupart);
	       double CNalphar3 = dperp->GetCNdiff_alpha3_r(ind, nupart);
		  ...
		  Utility::solve_tridag(lodrr, drr, uodrr, Rrr, xx, dimr);
		     ...
		     //if ((uid == -997) & (value > 1.e-45)) // || uid == -998)
		     //cout << "\nHere is the 997 value in xx!:" <<  value << endl;
      ...
      //if (uid == -997 || uid == -998)
      //for (int k = 0; k < dimE; k++) cout << "\nHere is the density!:" <<  N[index(int(dimr/2),int(dimz/2),k)] << " of nucleus: " << uid << endl;
   } // while (dt > dtmin)
...
double TCREvolutorADI::FindMax(TDiffusionCoefficient* dperp) {
   ...
   vector<double> spectrum = dperp->GetSpectrum(nupart);
...
double TCREvolutorADI::FindTRiacc(TReaccelerationCoefficient* dpp, vector<double>& totmomentum) {
   ...
   for (int i = 1; i < dimE; i++) {
      aux = pow(totmomentum[i]-totmomentum[i-1],2.0)/dpp->GetSpectrum(i, nupart);
...
//modified
void TCREvolutorADI::Run(vector<double>& N, vector<double>& N_previous,  TInelasticCrossSection *xsec_object, const vector<double>& dpdt, const vector<double>& SecSource_, const vector<double>& spectrum, double A, double Z, double lifetime, int daughter, std::vector<int>& nuclei, bool SecEl, int K_electron, bool isDM, bool isextra) {
...
TCREvolutor3D::TCREvolutor3D(Galaxy* gal1)  {
   ...
   gal = gal1;
   in = gal1->GetInput();
   mynuclist = (gal1->GetNucList())->GetList();
   coord = gal->GetCoordinates();
...
void TCREvolutor3D::Run(vector<double>& N, vector<double>& N_previous, TInelasticCrossSection *xsec_object, const vector<double>& dpdt, const vector<double>& SecSource_, const vector<double>& spectrum, double A, double Z, double lifetime, int daughter, std::vector<int>& nuclei, bool SecEl, int K_electron, bool isDM, bool isExtra) {
  ...
  if (Z > 0) uid = 1000*Z+A;
   ...
   int nupart;
   //std::vector<int> nuclei = mynuclist;//->GetList();
   for (int nuloop = 0; nuloop < mynuclist.size(); ++nuloop){
     if(uid == mynuclist[nuloop]) nupart = nuloop;	
   }
   ...
   if (A == 0 && Z == -1) {
      ...
      vector<double> dperp_vec = dperp->GetSpectrum(nupart);
  ...
  if (in->feedback >0) cout << "We are starting the 3D ISOTROPIC engine. Diffusion coefficient is a scalar, propagation is computed in 3+1D grid (x,y,z,p)." <<endl;

   cout << "injection spectrum" << endl;
   ...
   */
   for (int ip = 0; ip < dimE; ip++){  dppfactor[ip]=1.0; dperpfactor[ip]=1.;}
      ...
      for (j = 0; j < dimy; j++) {
         for (k = 0; k < dimz; k++) {
            ...
            for (ip = 1; ip < dimE-1; ip++) {
               ...
               if (dpp) {
                  double dppfix = dpp1*dpp->GetSpectrum(ip, nupart);
                  double dppdown = dpp1*dpp->GetSpectrum(ip-1, nupart);
	       ...
	       for  (k = 1; k < dimz-1; k++) {
                  for (j = 1; j < dimy-1; j++) {
                     ...
                     for (i = 0; i < dimx;  i++) {
                        ...
                        double CNalphax1 = dperp->GetCNdiff_alpha1_x(ind, nupart);
                        double CNalphax2 = dperp->GetCNdiff_alpha2_x(ind, nupart);
                        double CNalphax3 = dperp->GetCNdiff_alpha3_x(ind, nupart);
               ...
               for  (k = 1; k < dimz-1; k++) {
                  for (i = 1; i < dimx-1; i++) {
                     ...
                     for (j = 0; j < dimy;  j++) {
                        ...
                        double CNalphay1 = dperp->GetCNdiff_alpha1_y(ind, nupart);
                        double CNalphay2 = dperp->GetCNdiff_alpha2_y(ind, nupart);
                        double CNalphay3 = dperp->GetCNdiff_alpha3_y(ind, nupart);
