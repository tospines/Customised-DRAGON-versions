...
A(A_),
Z(Z_),
uid((Z_<-1 & A!=0) ? (-1000+A_) : (1000*Z_+A_)), 
...
K_electron(K_electron_) {
  ...
  if (in->DiffT == Anisotropic) {
   ...
   if (issec) sp = new TSpectrum(gal->GetCoordinates()); // Secondary particles
	 ...
	 if (A==3 && Z==-2)	
	   sp = new TSpectrum(gal->GetCoordinates(), in->MySelfTableDMHe3bar, 0);
	 if (A==4 && Z==-2)	
	   sp = new TSpectrum(gal->GetCoordinates(), in->MySelfTableDMHe4bar, 0);
       }
     }
   }
...
void TParticle::Evolve(vector<TParticle*> part, vector<TCREvolutorBasis*> crev_, TSpallationNetwork* spnet, vector<TXSecBase*> xsecmodel, bool isSecondIteration ) {
   ...
   //if (uid == -997 || uid == -998){ std::cout << secsource[TParticle::index(int(dimz/2),int(dimz/2),i)] << " to " << uid << " \n\n " << std::endl;}
   ...
   if (in->DiffT == Isotropic) {
     ...
     /*
       if (uid == -997 || uid == -998){
       for (int i = 0; i < dimE; ++i)
       if (density[index(int(dimz/2),int(dimz/2),i)] > 1e-30) std::cout << "  Density!\n\n  " << density[index(int(dimz/2),int(dimz/2),i)] << std::endl;}
     */
...
#undef DEBUG
   ...
   if (issec && fabs(uid) != 1000) { // Secondary protons, tertiary antiprotons, tertiary antideuterons and tertiary antiHe
	 ...
	 if ( in->spallationxsec == Fluka){//Fluka model
	   ...
	   for (int k = 0; k < dimr; ++k) {
		 ...
		 //integration over energy of primary particles! ii is equal or bigger than i (the energy of the primary can never be smaller!)
     ...
     else if (uid == -998 && issec){
       cout << "Tertiary anti-Deuterons" << endl;
       cout << "*** DRAGON Tertiary Anti-D procedure ***" << endl;
	 ...
	 for (int l = 0; l < dimz; ++l) {
	   ...
	   for (int i = 0; i < dimE; ++i)  {
	     ...
	     int ind = indspat*dimE+i;
	     for (int ii = i; ii < dimE; ++ii){ result[ind] += (*iprim)->GetDensity(indspat*dimE+ii) * betaigasdensity * spall_spectrum[ii];}
	     //std::cout << "\ntertiary Anti-D " << result[ind] << std::endl;
	   }
	 }
       }

       /*for (int i = 0; i < dimE; ++i)
	 if ((*iprim)->GetDensity(coord->index(int(dimr/2),int(dimz/2))*dimE+i) > 1e-30) std::cout << "  Density!\n\n  " << (*iprim)->GetDensity(coord->index(int(dimr/2),int(dimz/2))*dimE+i) << std::endl;
       */
     }//End Tertiary Anti-D

     
     else if (uid == -997 && issec){
       cout << "Tertiary anti-He3" << endl;
       cout << "*** DRAGON Tertiary Anti-He3 procedure ***" << endl;
	 ...
	 for (int l = 0; l < dimz; ++l) {
	   ...
	   for (int i = 0; i < dimE; ++i)  {
	     ...
	     int ind = indspat*dimE+i;
	     for (int ii = i; ii < dimE; ++ii){ result[ind] += (*iprim)->GetDensity(indspat*dimE+ii) * betaigasdensity * spall_spectrum[ii]; }
	     //std::cout << "\ntertiary Anti-He3 " << (*iprim)->GetUid() << " " << (*iprim)->GetDensity(indspat*dimE+i) << std::endl;
	   }
	 }
       }
       //for (int i = 0; i < dimE; ++i)
       //if ((*iprim)->GetDensity(coord->index(int(dimr/2),int(dimz/2))*dimE+i) > 1e-30) std::cout << "  Density!\n\n  " << (*iprim)->GetDensity(coord->index(int(dimr/2),int(dimz/2))*dimE+i) << std::endl;
       //exit(-1);
     }//End Tertiary Anti-He3
     
     /*
     else if (uid == -996 && issec){
       cout << "Tertiary anti-He4" << endl;
       cout << "*** DRAGON Tertiary Anti-He4 procedure ***" << endl;
	 ...
	 for (int l = 0; l < dimz; ++l) {
	   ...
	   for (int i = 0; i < dimE; ++i)  {
	     ...
	     int ind = indspat*dimE+i;
	     for (int ii = i; ii < dimE; ++ii){ result[ind] += (*iprim)->GetDensity(indspat*dimE+ii) * betaigasdensity * spall_spectrum[ii];}
	     //std::cout << "\ntertiary Anti-He4 " << result[ind] << std::endl;
	   }
	 }
       }
     }//End Tertiary Anti-D
     ...
     if (K_electron > 0) {
      ...
      //if K_capture is present and K_electron == 1, the source term ends here: it only contains contribution from the corresponding naked nucleus
	   ...
	   if (spall_spectrum.size() == dimE || (*ipart)->GetDaughter() == uid) {
	     if (spall_spectrum.size() == dimE){
	       for (int i = 0; i < dimE; ++i) {
		 if ( uid == 5011 || uid == 5010) spall_spectrum[i] *= in->BXS_Scale;
		 if ( uid == 4010 || uid == 4009 || uid == 4007)  spall_spectrum[i] *= in->BeXS_Scale;
		 if ( uid == 3007 || uid == 3006)  spall_spectrum[i] *= in->LiXS_Scale;
		 if ( uid == 9019 || uid == 9018)  spall_spectrum[i] *= in->FXS_Scale;
	       }
	     }
   ...
   else { // Secondary antiprotons, electrons and positrons, antideuterons, antiHe, positrons from TPP
     
      if (!isTPP) {
	vector<double> beta;
	if (fabs(uid) == 1000) beta = coord->GetBetaEl();
	else beta = coord->GetBeta();
	
	for (vector<TParticle*>::iterator ipart = part.begin(); ipart != part.end()-1; ++ipart) {
	  if ((*ipart)->GetUid() > 28014) continue;  //(*ipart)->GetUid() > 2004 || (*ipart)->GetUid() < 0)
	  for (int i = 0; i < dimE; i++) {
	    vector<double> spall_spectrum( spnet->GetXSecApEl((*ipart)->GetUid(), uid, i) );
		...
		for (int l = 0; l < dimz; l++) {
		  ...
		  int ind = indspat*dimE+i;
		  // ENERGY INTEGRAL --> eqs. 3.10 and 3.11 of DRAGON2-2
...
vector<double> TParticle3D::ComputeSecondarySource(vector<TParticle*> part, TSpallationNetwork* spnet, vector<TXSecBase*> xsecmodel) {
   ...
   if (issec && fabs(uid) != 1000) { // Secondary protons or tertiary antiprotons or tertiary antideuterons: they are next to their primary brothers...
     ...
     for (vector<TParticle*>::iterator ipart = part.begin(); ipart != part.end()-1; ++ipart) {
       if ((*ipart)->GetUid() > 28014 || (*ipart)->GetUid() < 0) continue;
       for (int i = 0; i < dimE; i++) {
	 vector<double> beta;
	 if (fabs(uid) == 1000) beta = coord->GetBetaEl();
	 else beta = coord->GetBeta();
	 ...
	 if (spall_spectrum.size() == dimE) {
	   for (int k = 0; k < dimx; k++) {
	     for (int j = 0; j < dimy; ++j) {
	       for (int l = 0; l < dimz; l++) {
		 int indspat = coord->indexD(k,j,l);
		 double gasdensity = totalgas->GetGas(indspat)*beta[i];
		 int ind = indspat*dimE+i;
		 // ENERGY INTEGRAL --> eqs. 3.10 and 3.11 of DRAGON2-2
